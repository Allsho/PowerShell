...
function Write-DebugLog {# MappingDebug.ps1
...
# Function to write debug messages to log
function Write-MappingDebugLog {
    param ([string]$Message)
    $timestamp = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
    Add-Content -Path $LogFilePath -Value "$timestamp - $Message"
}

# Function to retrieve and validate data mappings
function Validate-MappingConfig {
    param (
        [string]$ServerName,
        [string]$DatabaseName,
        [string]$Query,
    ...
    Write-MappingDebugLog "Starting data mapping retrieval from database..."
    try {
        ...
        if ($null -eq $mappingConfig) {
            ...
            Write-MappingDebugLog "Mapping configuration is null."
            exit
        } elseif ($mappingConfig.Rows.Count -eq 0) {
            Write-Host "Warning: No data found in the mapping configuration."
            Write-MappingDebugLog "No data found in the mapping configuration."
            exit
        } else {
            Write-Host "Mapping configuration retrieved. Row count: $($mappingConfig.Rows.Count)"
            Write-MappingDebugLog "Mapping configuration retrieved. Row count: $($mappingConfig.Rows.Count)"
        }
    } catch {
        Write-Host "Error: Failed to retrieve mapping configuration. $_"
        Write-MappingDebugLog "Failed to retrieve mapping configuration. $_"
        exit
    }

    Write-MappingDebugLog "Validating data mapping indices..."
    try {
        $rowIndex = 0
        ...
        if ($rowIndex -ge $mappingConfig.Rows.Count) {
            ...
            Write-MappingDebugLog "Row index $rowIndex is out of bounds. Mapping row count: $($mappingConfig.Rows.Count)"
        } elseif ($null -eq $mappingConfig.Rows[$rowIndex][$columnIndex]) {
            Write-Host "Error: Value at Row $rowIndex, Column '$columnIndex' is null."
            Write-MappingDebugLog "Value at Row $rowIndex, Column '$columnIndex' is null."
        } else {
            Write-Host "Value at Row $rowIndex, Column '$columnIndex' is valid."
            Write-MappingDebugLog "Value at Row $rowIndex, Column '$columnIndex' is valid: $($mappingConfig.Rows[$rowIndex][$columnIndex])"
        }
    } catch {
        Write-Host "Error: Indexing operation failed. $_"
        Write-MappingDebugLog "Indexing operation failed. $_"
    }
